...
import QuarterlyReputationTrendChart from "@/components/charts/QuarterlyReputationTrendChart";
...
function CardQuarterlyLine({ title, subtitle, data, onClick, ...props }) {
  ...
  const chartData = data.reputationSentimentTrendData;

  if (!chartData.seriesData || chartData?.seriesData?.length === 0) {
    return <CardSkeletonEmpty title={title} subti tle={subtitle} />;
  }
  ...
  const handleClick = ({ category, options, series }) => {
    ...
    const foundTag = chartData.filter((item) =>
      item.intervals.some(
...
export default CardQuarterlyLine;
