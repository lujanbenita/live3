...
import { tagSelectedToFirstPosition } from "@/utils/tagSelectedToFirstPosition";
...
const InputSearchRI = ({ name, control }) => {
  ...
  useEffect(() => {
    if (!selectedTag && value.length > 0) {
      handleSelectTag(value[0]);
    }
    setPlaceholder(value.length === 0 ? "Add Target" : "Add Peers");
    setShowTutorial(value.length === 0);
  }, [value]);
    ...
    <ClickAwayListener onClickAway={() => setOpen(false)}>
      <div className="c-input-search__absolute-position-reference">
        <div className="c-input-search" aria-hidden="true" onClick={focusInput}>
          ...
          >
            {value &&
              tagSelectedToFirstPosition(value, selectedTag).map((tag) => (
                <Tag
                  key={tag.tagName || tag}
                  tag={tag}
          ...
          {options.length > 0 && open ? (
            <div className="c-input-search__list-box" ref={optionsContainer}>
              {options
                .filter(
                  (option) =>
                    option.tagTypeName === "Company" ||
                    option.tagTypeName === "Person"
                )
                .map((option) => (
                  <InputSearchOption
                    key={option.tagId}
        ...
        <Steps
          initialStep={0}
          enabled={value.length === 0 && showTutorial}
          steps={[
            {
              element: ".c-input-search__input-container",
              intro: `
                    <div class="c-tutorial__text">
                      <p>A target company is required to perform a search.</p>
                      <br />
                    </div>
                  `,
              tooltipClass:
                "c-tutorial__tooltip c-tutorial__tooltip--dashboard-4",
              highlightClass:
                "c-tutorial__highlight c-tutorial__highlight--enabled-events",
              position: "bottom",
            },
          ]}
          options={{
